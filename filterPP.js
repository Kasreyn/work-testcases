// g++ -E -P -std=c++20 -I. -isystem oe-workdir/recipe-sysroot-native/ Common/Spaces-source.hpp | node ~/stash/testcases/filterPP.js | clang-format | less
...
function removeNamespaces1(input) {
...
function removeNamespaces2(input) {
	let insideNamespace = false;
	let braceDepth = 0;
	const lines	= input.split('\n');
	...
	const filtered = lines.filter(line => {
        ...
        if (line.includes('namespace __gnu_debug') || line.includes('struct') || line.includes('struct sched_param')) {
            insideNamespace = true;
            braceDepth = (line.match(/{/g) || []).length - (line.match(/}/g) || []).length;
...
function applyFilters(input) {
	...
	output	   = removeNamespaces1(output);
	...
	output	   = removeNamespaces2(output);
	return output;
}
