...
template<int Level>
void call_without_exception() {
	if constexpr (Level == 0) {
	}
	else {
		call_without_exception<Level - 1>();
	}
}

template<int Level>
ErrorCode call_without_error_return_code() {
	if constexpr (Level == 0) {
		return ErrorCode::Success;
	}
	else {
		ErrorCode success = call_without_error_return_code<Level - 1>();
		if (success == ErrorCode::Failure) {
			return ErrorCode::Failure;
		}
		else {
...
void test_return_code_handling() {
	...
	if (call_with_return_code<Levels>() == ErrorCode::Failure) {
		std::cout << "Error occurred at some level" << std::endl;
	}
...
template<int Levels>
void test_no_exception_handling() {
	auto start = std::chrono::high_resolution_clock::now();
	
	try {
		call_without_exception<Levels>();
	} catch (const std::runtime_error& e) {
...
template<int Levels>
void test_no_error_return_code_handling() {
	auto start = std::chrono::high_resolution_clock::now();

	if (call_without_error_return_code<Levels>() == ErrorCode::Failure) {
...
int main() {
	...
	test_no_exception_handling<levels>();

	std::cout << "\nSuccess path: Testing with return codes (" << levels << " levels):" << std::endl;
	test_no_error_return_code_handling<levels>();
